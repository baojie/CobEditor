...
public class MOEditor extends MOEditorGui implements MessageHandler {
	...
	public void updateUser(User newUser) {
		if (MOEditor.user == newUser) {
			return;
		}
		// report the log off the current user
		if (MOEditor.user != null) {
		...
		if (newUser != null) {
			// cancel all editing by this user [may cause by incorrect exiting
			// of last run]
	...
	public void onCancalAllEditing(ActionEvent e) {
		Vector<String> package_oid = UserManager.getEditingPackages(conn.db,
				user.name, true);
		Vector<String> package_pid = UserManager.getEditingPackages(conn.db,
				user.name, false);
		if (package_oid.size() == 0) {
			...
			Debug
					.trace("You cancel the editing privilege for following packages:\n "
							+ package_pid);
		}
	}
	...
	public void onAbout(ActionEvent e) {
		...
		String infoAbout = "<html>"
				+ "<font color=\"#FF0099\"><b>"
				+ AtoConstent.APP_NAME
				+ "</b></font><br>Version "
				+ 2.1
				+ "<br>"
				+ "<br><b>Jie Bao, Peter Wong, LaRon Hughes </b><br>2005-2006<br>"
	...
	public boolean load(OntologyServerInfo info) {
		...
		int answer = JOptionPane.showConfirmDialog(null, "Load ontology '"
				+ info.name + "'?");
	...
	public void onUserMgmt(ActionEvent e) {
		if (user.isAdmin()) {
			UserManagementCenter dlg = new UserManagementCenter(this.mainFrame,
					this.selectedServer);
			GUIUtils.centerWithinParent(dlg);
			dlg.setVisible(true);
		} else {
